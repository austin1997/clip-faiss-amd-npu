...
parser.add_argument(
    '--data_dir',
    default="./Datasets/ye-pop/images/chunk_1",
    help='Data set directory, when quant_mode=calib, it is for calibration, while quant_mode=test it is for evaluation')
parser.add_argument(
    '--model_dir',
    # default="./Models/CLIP",
    # default="./Models/open-clip/CLIP-ViT-B-32-laion2B-s34B-b79K/open_clip_pytorch_model.bin",
    default="./onnx/RN50/image_model.onnx",
...
def quantization(title='optimize',
  ...
  if file_path.endswith('.onnx'):
    import onnx2torch
    model = onnx2torch.convert(file_path)
    print(model)
    input = torch.randn([batch_size, 3, 224, 224])
    # import pdb; pdb.set_trace()
    print(model(input))
  else:
    model, preprocess = clip.load(model_name, device=device, download_root=file_path)
    model.forward = model.encode_image
    
    # model = torch.fx.experimental.optimization.optimize_for_inference(model)
    # model, _, preprocess = open_clip.create_model_and_transforms(model_name, pretrained=file_path, cache_dir="./model_cache")
  model.eval()
  input = torch.randn([batch_size, 3, 224, 224])
  if quant_mode == 'float':
    quant_model = model
